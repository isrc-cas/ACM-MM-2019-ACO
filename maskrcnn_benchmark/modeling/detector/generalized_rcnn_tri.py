...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.target_rpn = build_rpn(cfg, self.backbone.out_channels)
        self.target_roi_heads = build_roi_heads(cfg, self.backbone.out_channels)
        # ----------------------------------------------------
        # ----------------------------------------------------
        # ----------------------------------------------------
    ...
    def init_weights_from_trained_head(self):
        self.target_rpn.load_state_dict(self.rpn.state_dict())
        self.target_roi_heads.load_state_dict(self.roi_heads.state_dict())

    def forward(self, images, targets=None, train_target=False):
        """
        Arguments:
        ...
        if self.first_run:
            ...
            self.init_weights_from_trained_head()
            print('init from another head!!!!!!!!!!!')
        ...
        density_loss = None
        predication_density_map = None

        if train_target or self.run_inference:  # inference is only on target domain
            # ----------------------------------------------------
            # -------------- target domain training --------------
            # ----------------------------------------------------
            proposals, proposal_losses = self.target_rpn(images, features, targets)
            if self.roi_heads:
                x, result, detector_losses = self.target_roi_heads(features, proposals, targets)
            else:
        ...
        if predication_density_map is None:
            return result
        else:
            final_result = []
