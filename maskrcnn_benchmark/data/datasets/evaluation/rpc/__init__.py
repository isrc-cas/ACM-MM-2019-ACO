...
def rpc_evaluation(dataset, predictions, output_folder, box_only, **_):
    ...
    # print('Pseudo-Labeling: {}'.format(len(anns)))
...
def rpc_evaluation_v1(dataset, predictions, output_folder, box_only, **_):
    if not predictions[0].has_field('density_map'):
        print('no density_map evaluation')
        return __rpc_evaluation(dataset, predictions, output_folder, box_only, **_)
    print('density_map evaluation')
    pred_boxlists = []
    annotations = []
    correct = 0
    mae = 0
    ...
    with open(os.path.join(output_folder, 'result_{}.txt'.format(datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))), 'w') as fid:
        fid.write(str(result) +
                  '\n' +
                  'Ratio: {}, MAE: {:.3f}, Pseudo-Labeling: {}'.format(correct / len(predictions), mae / len(predictions), len(annotations)))
